...
from datetime import date, datetime, timedelta # Se agrego timedelta
...
def nomina_mensual(request):
    ...
    if fecha_get:
        ...
        # LÓGICA DE MES ANTERIOR POR DEFECTO
        # Calculamos el primer día del mes actual y restamos 1 día para obtener el mes anterior
        primer_dia_mes_actual = hoy.date().replace(day=1)
        fecha_mes_anterior = primer_dia_mes_actual - timedelta(days=1)
        fecha_seleccionada = fecha_mes_anterior.replace(day=1)
...
def eliminar_liquidacion(request, id):
    """
    Elimina una liquidación existente y redirige a la nómina del periodo correspondiente.
    """
    remu = get_object_or_404(Remuneracion, id=id)
    periodo_actual = remu.periodo # Guardamos el periodo para el redirect
    
    # Eliminar el registro
    remu.delete()
    
    # Redirigir a la nómina con el filtro de fecha activo
    url = reverse('nomina_mensual')
    return redirect(f"{url}?fecha={periodo_actual}")
